...
class DEIMSIso19139Controller extends ControllerBase
{
    public function Iso19139Response($record_information)
    {
        ...
        $charStr = $doc->createElement(
            "gco:CharacterString",
            $record_information["id"]["suffix"] ?? "default-id"
        );
        ...
        if (
            !empty($record_information["attributes"]["contact"]["siteManager"])
        ) {
            foreach (
                $record_information["attributes"]["contact"]["siteManager"]
                as $person
            ) {
        ...
        if (
            !empty(
                $record_information["attributes"]["contact"][
        ...
        $stdName->appendChild(
            $doc->createElement("gco:CharacterString", "ISO 19115:2003/19139")
        );
        $root->appendChild($stdName);

        $stdVersion = $doc->createElement("gmd:metadataStandardVersion");
        $stdVersion->appendChild(
            $doc->createElement("gco:CharacterString", "1.0")
        );
        ...
        $dateVal = $doc->createElement(
            "gco:Date",
            $record_information["changed"]
        );
        ...
        $abstractText->appendChild(
            $doc->createTextNode(
                $record_information["attributes"]["general"]["abstract"] ??
        ...
        $licenseText =
            "This dataset is licenced under the Creative Commons Attribution 4.0 International (CC BY 4.0). See https://creativecommons.org/licenses/by/4.0/";
        $useLimitation->appendChild(
            $doc->createElement("gco:CharacterString", $licenseText)
        );
        ...
        if ($record_information["attributes"]["general"]["images"]) {
            ...
            $fileName->appendChild(
                $doc->createElement("gco:CharacterString", $image["url"])
            );
            $browseGraphic->appendChild($fileName);

            // Optional description
            $fileDesc = $doc->createElement("gmd:fileDescription");
            $fileDesc->appendChild(
                $doc->createElement("gco:CharacterString", $image["alt"])
            );
        ...
        if ($record_information["attributes"]["geographic"]["boundaries"]) {
            $bbox_geom = $this->getBoundingBoxFromWKT(
                $record_information["attributes"]["geographic"]["boundaries"]
            );
            ...
            $wkt =
                $record_information["attributes"]["geographic"]["coordinates"];
            preg_match(
                "/POINT\s*\(\s*([\d\.\-]+)\s+([\d\.\-]+)\s*\)/i",
                $wkt,
                $matches
            ...
            $gmlPoint = $doc->createElementNS(
                "http://www.opengis.net/gml",
                "gml:Point"
            );
            $gmlPoint->setAttribute(
                "gml:id",
                "centroid_or_representative_coordinates_of_site"
            );
            $gmlPoint->setAttribute(
                "srsName",
